...
Notes: Implemented exactly as planned. `gameState.test.ts` gained a
`describe` block (`buildGameConfiguration("clash")` from `games.ts`) pinning
that `buildInitialGameState` stamps the exact three-token tag `2-0:BATTLE
ARMY_COMPOSITION=standard_clash BOARD_LAYOUT=asymmetric_100` and that
`renderPositionBlock` renders 10 lines of 10 three-character cells with `XXX`
at columns A/D/G/H/I on rows 5 and 6 only (checked positively for the five
lake columns and negatively for the five open columns, plus a scan
confirming no other row contains an `XXX` cell). `readRecord.test.ts` gained:
a case driving the real writer (`startPlay`/`applyMove`/`renderGameRecord`)
on a Clash configuration through a Champion-captures-Flag game, asserting the
resolved `boardLayout.id`/`ARMY_COMPOSITION`/`army`, zero unrecognized
tokens, and the full replayed move list; a case composing Clash with a
deviating `DIAGONAL_ATTACKABLE=all`, pinning token order (both game-defining
flags before the diagonal one) and that the two kinds of flag compose; a
canonicalization case for `2-0:BATTLE BOARD_LAYOUT=standard_144
ARMY_COMPOSITION=standard_battle` reading as the plain standard Battle
configuration; three cases pinning that the bare `2-0:BATTLE`, `2-1:SKIRMISH`
and `2-0:SKIRMISH` tags still read with no tokens and no deviation; and a
case reading the new checked-in sample from disk and replaying it. Added
`doc/samples/2-0-battle-clash-flag-capture.txt` — a short Clash game (White
Champion at E5 attacks the Black Flag at E6 across the open E-F lane,
capturing it and ending the game) — generated by calling the real writer
(`buildGameConfiguration("clash")`/`startPlay`/`applyMove`/`renderGameRecord`)
from a throwaway script and checking in its exact output, rather than
hand-typing the text, per the orchestrator's instruction; the script was
deleted after use and is not part of the commit. `doc/samples/README.md`
gained a matching entry, explaining in-line why the tag names Battle (no
published Clash edition exists; the app records Clash as a deviation from
`2-0:BATTLE`) and noting the file was hand-built to match the writer's exact
output because the picker cannot start a Clash game until Step 9. No
deviation from the plan. All five repository checks (typecheck, lint, test —
838 passed, up from 829 — format:check, build) are clean; `git diff --stat
doc/samples` shows only the `README.md` addition and the one new sample
file, and `git diff --stat -- src/engine src/encoding` is empty.
