...
- **A fourth game on the "Choose a game" screen**, named **Demotion**,
  described as: *"Play on an 8x8 fixed board. When a piece wins a battle, it
  is demoted one rank."* Offered in the same plain language as the other
...
- **Major 3 is specified in that repository's `doc/ruleset/proposed-3/`
  ...
  - `changelog.md` — the entry major 3 *would* publish, written as the delta
...
- Both branches produce an even position; what the rule chooses is that each
  Flag faces the *weaker* part of the enemy army either way.
...
- **Two squares in a straight orthogonal line** if the piece is *unencumbered
  in the direction it is moving* and the square passed through is empty. May
...
- **Encumbrance is direction-relative**: a piece is encumbered for a given
  ...
  another. *(Major 2 encumbered on any of the eight surrounding squares, in
  every direction.)*
...
least one of the two squares orthogonally adjacent to *both* attacker and
...
The simplified form `A2A4` remains available for *entering* a move in a text
...
- **Loss — Attrition**: a player left with **no numbered pieces** loses
  immediately. The Flag does not count. *(Replaces major 2's "no legal move",
  and is checked after every ply rather than at the start of a turn.)*
...
- **Draw — Inactivity**: a counter from 0, +1 on every move that removes no
  piece, reset to 0 by any removal; **a draw at 40**. *(Major 2's limit is
  50.)* A move that only reduces a rank without removing anything cannot
  occur.
...
Both are quoted here because they produce a *wrong board* rather than an
...
- `src/rules/readRecord.ts` — the version-dispatch entry point for reading a
  record. Its own header already anticipates this story: *"A future ruleset
  version adds a case here rather than editing an existing one."* Today it
...
- **The player-facing name is "Demotion"**, and its description is exactly:
  *"Play on an 8x8 fixed board. When a piece wins a battle, it is demoted one
  rank."*
...
2. **Rank as mutable state.** A piece on the board is a side and a *current*
...
8. **Piece art keyed by `(major, rank)`.** `PieceIcon` cannot keep taking a
   ...
   the major *and* the current rank digit — never from a name, never from a
...
12. **Everything keeps working, accessibly.** A Demotion game is fully
    ...
    that are new to announce: a piece's rank *changing* mid-game, and a game
...
- **Rank reduction is the deepest new idea, and it is not a display concern.**
  ...
  state and every announcement have to read *current* rank. This is the single
...
- **`(major, rank digit)` is the keying rule, and the proposal is emphatic
  about it.** Reusing sprites by piece *name* across majors is precisely the
...
- **The sprites are settled** (owner decision at story time). Ranks 2–5 reuse
  ...
  for a sapper. `p-tower`, `p-knight` and `p-halberdier` are not used at major
  3.

  Note what this is and is not. Four of the five sprites are shared with major
  2 **by name**, because the artwork suits the name — but they are *keyed* by
...
- **The generated position replaces a phase, not just a screen.** Removing
  ...
  A Demotion game starts at what is currently the *second* branch of
...
- **Gate D — Movement.** One-square orthogonal moves work everywhere;
  ...
  unencumbered *in that direction* — demonstrably free forwards with an enemy
...
- **How `PieceIcon` is re-keyed** to `(major, rank)` without changing what
  major 2 renders today. *Which* sprite each major-3 rank uses is already
