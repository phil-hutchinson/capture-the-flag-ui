...
Story 00000030 ("Implement 10x10 board"), Step 7. A short, complete Clash
game (`BOARD_LAYOUT=asymmetric_100`, `ARMY_COMPOSITION=standard_clash`) on
the 10×10 board: a White Champion at E5 attacks the Black Flag standing
orthogonally at E6, across the lake rows' open E–F lane, capturing it
outright and ending the game.

**Its `Ruleset` tag names Battle** — `2-0:BATTLE ARMY_COMPOSITION=standard_clash
BOARD_LAYOUT=asymmetric_100` — because there is no published Clash edition
for this app to invent one: story.md's Policy has a Clash game record as a
deviation from `2-0:BATTLE` instead, the two flags naming the actual board
and army played (see this story's Grounding facts, "How Clash is recorded").

Like the three diagonal-flag fixtures above, this file was **hand-built to
match the writer's exact output** — its text was generated by calling the
real writer (`buildGameConfiguration("clash")`/`startPlay`/`applyMove`/
`renderGameRecord`) directly and checking the result in, rather than played
through the app — because the new-game screen doesn't yet offer Clash as a
choice (that's Step 9; the picker can't start a Clash game until then). An
automated test (`src/rules/readRecord.test.ts`) reads this file from disk and
replays it to the end.
